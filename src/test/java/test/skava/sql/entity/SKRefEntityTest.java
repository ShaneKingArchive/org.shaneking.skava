...
public class SKRefEntityTest extends SKUnit {
  ...
  public void testQueryListSql() {
    ...
    Assert.assertEquals(addressEntity.selectSql().toString(), "(select address,postcode,primary,ref_type,ref_id,id,version,ext_json_str,create_datetime,create_user_id,last_modify_datetime,last_modify_user_id,invalid,invalid_datetime,invalid_user_id from testSchema.t_address where version=?,[1])");
    ...
    Assert.assertEquals(addressEntity.selectSql().toString(), "(select address,postcode,primary,ref_type,ref_id,id,version,ext_json_str,create_datetime,create_user_id,last_modify_datetime,last_modify_user_id,invalid,invalid_datetime,invalid_user_id from testSchema.t_address where address=? and id=? and version=?,[Junjin Load No.500, " + uuid + ", 1])");
  }
}
