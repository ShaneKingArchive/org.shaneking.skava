...
public class SKRefEntityTest extends SKUnit {
  ...
  public void testQueryListSql() {
    ...
    Assert.assertEquals(addressEntity.selectSql().toString(), "(select address,postcode,primary,ref_id,ref_type,create_datetime,create_user_id,ext_json_str,id,invalid,invalid_datetime,invalid_user_id,last_modify_datetime,last_modify_user_id,version from testSchema.t_address where version=?,[1])");
    ...
    Assert.assertEquals(addressEntity.selectSql().toString(), "(select address,postcode,primary,ref_id,ref_type,create_datetime,create_user_id,ext_json_str,id,invalid,invalid_datetime,invalid_user_id,last_modify_datetime,last_modify_user_id,version from testSchema.t_address where address=? and id=? and version=?,[Junjin Load No.500, " + uuid + ", 1])");
  }
}
