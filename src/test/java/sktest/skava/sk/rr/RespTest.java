...
import org.shaneking.skava.sk.rr.RespException;
...
public class RespTest extends SKUnit {
  ...
  public void failedA0() {
    Assert.assertEquals("{\"code\":\"-1\"}", OM3.writeValueAsString(Resp.failed()));
  }

  @Test
  public void failedA1() {
//    Assert.assertEquals("{\"code\":\"1\",\"mesg\":\"NULL\"}", OM3.writeValueAsString(Resp.failed(Resp.CODE_KNOWN_EXCEPTION)));
    Assert.assertEquals("{\"code\":\"-1\"}", OM3.writeValueAsString(Resp.failed(Resp.CODE_UNKNOWN_EXCEPTION)));
  }

  @Test
  public void failedA2() {
    Assert.assertEquals("{\"code\":\"-1\",\"mesg\":\"mesg\"}", OM3.writeValueAsString(Resp.failed(Resp.CODE_UNKNOWN_EXCEPTION, "mesg")));
  }

  @Test
  public void failedA3() {
    Assert.assertEquals("{\"code\":\"-1\",\"data\":\"data\",\"mesg\":\"mesg\"}", OM3.writeValueAsString(Resp.failed(Resp.CODE_UNKNOWN_EXCEPTION, "mesg", "data")));
  }

  @Test
  public void parseExp() {
    Assert.assertEquals("{\"code\":\"code\",\"mesg\":\"mesg\"}", OM3.writeValueAsString(Resp.<Integer>failed().parseExp(new RespException(Resp.failed("code", "mesg", "data")))));
    Assert.assertEquals("{\"code\":\"code\",\"mesg\":\"mesg\"}", OM3.writeValueAsString(Resp.<Integer>failed().parseExp(new RespException(Resp.failed("code", "mesg", "data")))));
  }
